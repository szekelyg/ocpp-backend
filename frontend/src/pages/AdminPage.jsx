...
const INPUT_CLS =
  ...
  "placeholder:text-slate-500 focus:border-blue-600 focus:outline-none";
...
function ChargerConfigModal({ cp, busy, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        <div className="px-5 py-4 border-b border-slate-700 flex items-center justify-between">
          <div>
            <div className="font-semibold text-slate-100">Töltő beállítása</div>
            <div className="font-mono text-xs text-slate-400">{cp.ocpp_id}</div>
          </div>
        ...
        <div className="px-5 py-4 border-t border-slate-700 flex flex-wrap gap-2 justify-end">
          <button
            onClick={onClose}
          ...
          <button
            ...
            className="rounded-lg border border-slate-600 bg-slate-700/40 px-3 py-1.5 text-sm text-slate-100 hover:bg-slate-700 disabled:opacity-40"
          >
            {busy ? "Mentés…" : "Mentés"}
          </button>
          {!cp.is_published && (
  ...
  function ChargerRow({ cp }) {
    return (
      <tr className="hover:bg-slate-800/30">
        ...
        <Td>
          <div>{cp.location_name || <span className="text-amber-400">nincs helyszín</span>}</div>
        ...
        <Td>{cp.connector_type || <span className="text-amber-400">—</span>}</Td>
        <Td>{cp.max_power_kw ? `${cp.max_power_kw} kW` : <span className="text-amber-400">—</span>}</Td>
        ...
        <table className="w-full">
          <thead className="bg-slate-800/60">
            <tr>
              ...
              <Th>Utoljára látva</Th><Th>Felvéve</Th><Th>Műveletek</Th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800">
      ...
      {pending.length > 0 && (
        <div className="rounded-xl border border-amber-700/40 bg-amber-900/10 p-4">
          ...
          <p className="text-xs text-slate-400 mb-3 max-w-3xl">
            Ezek a töltők felcsatlakoztak, de <span className="text-amber-300">még nem látszanak</span> az
