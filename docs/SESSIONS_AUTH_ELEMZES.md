...
|---|---|---|
| **Admin auth** – `verify_admin` (`admin.py`): 2026-09-23-tól Keycloak Bearer + `ev-admin` realm-szerep (403 `admin_role_missing` szerep nélkül); Basic (`ADMIN_USERNAME`/`ADMIN_PASSWORD`, `secrets.compare_digest`) csak vész-út, amíg az env be van állítva; egyik sem → 503 (fail-closed). Lásd docs/KEYCLOAK.md 6. | `/api/admin/*`, `/api/sessions/start`, `/api/sessions/stop`, **most már** `/api/sessions/` és `/active/by-charge-point` | AdminPage (`sessionStorage.admin_token`, Basic) |
